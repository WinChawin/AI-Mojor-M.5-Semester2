...
if __name__ == "__main__":
    ...
    try:
        # อ่านเป็น DataFrame ก่อนเพื่อตรวจสอบ column names
        df = pd.read_csv(data_path)
        print(f"Columns found: {list(df.columns)}")
        
        # ตรวจสอบชื่อ column ของ label
        label_col = None
        possible_label_names = ['label', 'Label', 'LABEL', 'target', 'Target', 'y', 'Y', 'class', 'Class']
        ...
        if label_col is None:
            # ถ้าไม่พบ ให้ใช้ column แรก
            label_col = df.columns[0]
            print(f"Warning: No standard label column found. Using first column '{label_col}' as label.")
        else:
            print(f"Using column '{label_col}' as label.")
        
        # แยก features และ labels
        Y_all = df[label_col].values.astype(int)
        X_cols = [col for col in df.columns if col != label_col]
        X_all = df[X_cols].values.T  # transpose เพื่อให้เป็น (features, samples)
        
        # สร้าง combined data สำหรับ shuffle
        combined_data = np.vstack([Y_all.reshape(1, -1), X_all])
        combined_data = combined_data.T  # (samples, features+1)
        np.random.shuffle(combined_data)  # สุ่มลำดับแถว
