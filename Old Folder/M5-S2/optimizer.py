import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import time 

def init_params():
    # He initialization for 4 layers with 100 nodes each
    W1 = np.random.randn(100, 784) * np.sqrt(2 / 784)
    b1 = np.zeros((100, 1))
    W2 = np.random.randn(100, 100) * np.sqrt(2 / 100)
    b2 = np.zeros((100, 1))
    W3 = np.random.randn(100, 100) * np.sqrt(2 / 100)
    b3 = np.zeros((100, 1))
    W4 = np.random.randn(10, 100) * np.sqrt(2 / 100)
    b4 = np.zeros((10, 1))
    return W1, b1, W2, b2, W3, b3, W4, b4

def relu(z):
    return np.maximum(z, 0)

def deriv_relu(z):
    return z > 0

def tanh(z):
    return np.tanh(z)

def tanh_derivative(z):
    return 1 - np.tanh(z)**2

def sigmoid(z):
    return 1 / (1 + np.exp(-z))

def sigmoid_derivative(z):
    s = sigmoid(z)
    return s * (1 - s)

def softmax(z):
    z = z - np.max(z, axis=0, keepdims=True)  # Numerical stability
    return np.exp(z) / np.sum(np.exp(z), axis=0, keepdims=True)

def compute_loss(A4, Y, W1, W2, W3, W4, lambd):
    m = Y.size
    one_hot_Y = one_hot(Y)
    log_probs = -np.log(A4[Y, np.arange(m)] + 1e-8)  # เพิ่ม epsilon เพื่อป้องกัน log(0)
    data_loss = np.sum(log_probs) / m
    # Add L2 regularization
    reg_loss = (lambd / (2 * m)) * (np.sum(W1**2) + np.sum(W2**2) + np.sum(W3**2) + np.sum(W4**2))
    return data_loss + reg_loss

def forward_prop(W1, b1, W2, b2, W3, b3, W4, b4, X):
    Z1 = W1.dot(X) + b1
    A1 = relu(Z1)
    Z2 = W2.dot(A1) + b2
    A2 = relu(Z2)
    Z3 = W3.dot(A2) + b3
    A3 = relu(Z3)
    Z4 = W4.dot(A3) + b4
    A4 = softmax(Z4)
    return Z1, A1, Z2, A2, Z3, A3, Z4, A4

def back_prop(Z1, A1, Z2, A2, Z3, A3, Z4, A4, W1, W2, W3, W4, X, Y, lambd):
    m = Y.size
    one_hot_Y = one_hot(Y)
    
    # Layer 4 (output)
    dZ4 = A4 - one_hot_Y
    dW4 = 1/m * np.dot(dZ4, A3.T) + (lambd / m) * W4
    db4 = 1/m * np.sum(dZ4, axis=1, keepdims=True)
    
    # Layer 3
    dZ3 = np.dot(W4.T, dZ4) * deriv_relu(Z3)
    dW3 = 1/m * np.dot(dZ3, A2.T) + (lambd / m) * W3
    db3 = 1/m * np.sum(dZ3, axis=1, keepdims=True)
    
    # Layer 2
    dZ2 = np.dot(W3.T, dZ3) * deriv_relu(Z2)
    dW2 = 1/m * np.dot(dZ2, A1.T) + (lambd / m) * W2
    db2 = 1/m * np.sum(dZ2, axis=1, keepdims=True)
    
    # Layer 1
    dZ1 = np.dot(W2.T, dZ2) * deriv_relu(Z1)
    dW1 = 1/m * np.dot(dZ1, X.T) + (lambd / m) * W1
    db1 = 1/m * np.sum(dZ1, axis=1, keepdims=True)
    
    return dW1, db1, dW2, db2, dW3, db3, dW4, db4

def one_hot(Y):
    num_classes = 10  # แก้ให้เป็น 10 คลาสสำหรับ digits 0-9
    one_hot_Y = np.zeros((num_classes, Y.size))
    one_hot_Y[Y, np.arange(Y.size)] = 1
    return one_hot_Y

def update_params(W1, b1, W2, b2, W3, b3, W4, b4, dW1, db1, dW2, db2, dW3, db3, dW4, db4, alpha):
    W1 -= alpha * dW1
    b1 -= alpha * db1
    W2 -= alpha * dW2
    b2 -= alpha * db2
    W3 -= alpha * dW3
    b3 -= alpha * db3
    W4 -= alpha * dW4
    b4 -= alpha * db4
    return W1, b1, W2, b2, W3, b3, W4, b4

def get_accuracy(predictions, Y):
    return np.sum(predictions == Y) / len(Y)

def get_predictions(A4):
    return np.argmax(A4, axis=0)

def gradient_descent(X, Y, alpha, n_iters, batch_size, lambd):
    W1, b1, W2, b2, W3, b3, W4, b4 = init_params()
    m = X.shape[1] 
    accuracy_list = []
    loss_list = []
    iteration_list = []

    for i in range(n_iters + 1):
        # Mini-batch gradient descent
        for j in range(0, m, batch_size):
            X_batch = X[:, j:j+batch_size]
            Y_batch = Y[j:j+batch_size]

            Z1, A1, Z2, A2, Z3, A3, Z4, A4 = forward_prop(W1, b1, W2, b2, W3, b3, W4, b4, X_batch)
            dW1, db1, dW2, db2, dW3, db3, dW4, db4 = back_prop(Z1, A1, Z2, A2, Z3, A3, Z4, A4, W1, W2, W3, W4, X_batch, Y_batch, lambd)
            W1, b1, W2, b2, W3, b3, W4, b4 = update_params(W1, b1, W2, b2, W3, b3, W4, b4, dW1, db1, dW2, db2, dW3, db3, dW4, db4, alpha)

        # Monitor performance
        if i % 10 == 0:
            Z1, A1, Z2, A2, Z3, A3, Z4, A4 = forward_prop(W1, b1, W2, b2, W3, b3, W4, b4, X)
            predictions = get_predictions(A4)
            accuracy = get_accuracy(predictions, Y)
            loss = compute_loss(A4, Y, W1, W2, W3, W4, lambd) 
            accuracy_list.append(accuracy)
            loss_list.append(loss)
            iteration_list.append(i)
            print(f"Iteration {i}, Loss: {loss:.4f}, Accuracy: {(accuracy*100):.4f}")

        # Learning rate decay
        alpha = alpha * (1 / (1 + 0.001 * i)) 

    # Plot accuracy and loss
    plt.figure(figsize=(12, 5))
    plt.subplot(1, 2, 1)
    plt.plot(iteration_list, accuracy_list, label="Accuracy")
    plt.xlabel("Iterations")
    plt.ylabel("Accuracy")
    plt.title("Accuracy over Iterations")
    plt.grid()
    plt.legend()

    plt.subplot(1, 2, 2)
    plt.plot(iteration_list, loss_list, label="Loss", color="red")
    plt.xlabel("Iterations")
    plt.ylabel("Loss")
    plt.title("Loss over Iterations")
    plt.grid()
    plt.legend()

    plt.show()

    return W1, b1, W2, b2, W3, b3, W4, b4

# Load and preprocess data
data = pd.read_csv('digit-recognizer/train.csv').to_numpy()
np.random.shuffle(data)

m, n = data.shape
data_dev = data[:1000].T
Y_dev = data_dev[0].astype(int)  # แปลงเป็น int
X_dev = data_dev[1:] / 255.0

data_train = data[1000:].T
Y_train = data_train[0].astype(int)  # แปลงเป็น int
X_train = data_train[1:] / 255.0

# Train the model
W1, b1, W2, b2, W3, b3, W4, b4 = gradient_descent(X_train, Y_train, alpha=0.1, n_iters=1000, batch_size=64, lambd=0.1) 

# Evaluate on the development set
Z1, A1, Z2, A2, Z3, A3, Z4, A4 = forward_prop(W1, b1, W2, b2, W3, b3, W4, b4, X_dev)
dev_predictions = get_predictions(A4)
dev_accuracy = get_accuracy(dev_predictions, Y_dev)
print(f"Development Set Accuracy: {dev_accuracy:.4f}")